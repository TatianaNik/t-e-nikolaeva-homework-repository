...
def test_merge_sorted_files_mock(mocker):
    m = mocker.patch('file1.txt', return_value='1, 3, 5')
    n = mocker.patch('file2.txt', return_value='2, 4, 6')
    k = mocker.patch('file3.txt', return_value='7, 8, 9')
    assert(list(merge_sorted_files([m, n, k]) == [1, 2, 3, 4, 5, 6, 7, 8, 9]))
